/* "pokon0.c":어플리케이션 라운지  ver. 4.7
     copyright(C) 2004 I.Tak., 코야나기아명, 카와이 히데미
     stack:1m malloc:90k file:4096k */

/* scrollbar & mouse by I.Tak.  */
/* 시그널 수신 통지를*sbp==0 때에 1회만 한다……거의 의미 없음
   그리기 처리를 억제하기 위한 일보 전진입니다 */
...
#include <guigui00.h>
#include <sysgg00.h>
/* sysgg는, 일반의 어플리케이션이 이용해선 안 되는 라이브러리
   사양도 꽤 유동적 */
#include <stdlib.h>
...
void unlinkfbuf(struct FILEBUF *fbuf)
{
	...
	if (--fbuf->linkcount == 0) {
		...
		fbuf->dirslot |= -1;
		sgg_execcmd0(0x0074, 0, fbuf->virtualmodule, 0x0000); /* virtualmodule의 파기 */
		fbuf->readonly = 0;
	}
	return;
}
...
void runjobnext()
{
	...
	job_end:
		...
		case JOB_CHECK_WB_CACHE:
			if (check_wb_cache(filebuf) == NULL) /* 종료하면 NULL를 돌려준다 */
				goto job_end;
			break;
	...
	errorsignal:
		...
		case JOB_RESIZE_FILE:
			/* 2002.05. 28 사이즈 0의 파일을 단순하게 리사이즈 하면 쓰레기로 인해 혼란하게 하는 버그를 극복하기 위해서,
				2단계 사이즈 변경을 실시. 0→1→목적 사이즈. 게다가 1일때 바이트를 0xcc로 한다.  */
			...
			sgg_execcmd0(0x0020, 0x80000000 + 10, 0x1249, 0x0150,
				...
				0x4243 /* to pokon0 */, 0x7f000002, i, 1 /* 정상 종료가 -1이니까 */, 0x0000);
			break;
...
//			break;
		...
		case JOB_EXT_OPEN:
			...
			if (pjob->bank->tss >= 0x1000) {
				...
				vmr->sigbase = 16; /* 잠정 */
				vmr->flushed_size = 0; /* 잠정 */
				sgg_directwrite(0, 4, 0, i, /* 슬롯 i에 붙인다 */
...
lib_putstring_ASCII(0x0000, 0, 0, &selwin0[0]. subtitle.tbox, 0, 0, "debug! (2)");
		...
		case JOB_CHANGE_DRIVE:
			i = readjob();
			if (need_wb == 0) {
				sgg_execcmd0(0x009c, 0x190, i, 0x0000); /* 사실 이 방법은 좋지 않다 */
					/* 액세스가 경합 할지도 모르기 때문에 사실 시스템 태스크를 경유해야 함 */
		...
		case JOB_LOAD_AND_EXEC_PSF:
			...
			if (console->curx ! = -1) {
				pjob->now = 0; /* 콘솔을 열고 있었다 */
				break; /* 콘솔을 연 상태로 실행하면 보기 흉하기 때문에 */
			}
			if ((pjob->fp = searchfid((char *) &pjob->param[0])) == NULL) {
				pjob->now = 0; /* 파일이 없었다 */
				break;
		...
		case JOB_CHANGE_DEVICE:
			i = readjob();
			if (need_wb == 0) {
				sgg_execcmd0(0x009c, 0x194, i, 0x0000); /* 사실은 이 방법은 좋지 않다 */
					/* 액세스가 경합 할지도 모르기 때문에, 사실이라면 시스템 태스크를 경유해야 함 */
				#if (defined(PCAT))
...
void jsub_fbufready0(void *func)
/* pjob->fp로 나타나는 파일을 pjob->fbuf로 액세스 가능한 상태로 한다 */
/* 또한 이미 fbuf내에 존재하고 있었을 경우는 pjob->fbuf를 다시 수정한다(예비의 것은 자동 개방) */
/* 완료하면 pjob->retfunc를 부른다 */
/* 에러 때는 cond == 0이 된다. 에러의 경우 unlink의 필요는 없다 */
/* 에러시에는 pjob->fbuf->linkcount = 0;가 되어 있으므로, 필요하면 -1을 재대입하는 것 */
{
	struct STR_JOBLIST *pjob = &job;
	struct FILEBUF *fbuf;
	pjob->retfunc = func;
	...
	if (fbuf->size ! = -1) {
		...
		if (auto_decomp) {
			...
			if (size0 < FILEAREA / 2 && size0 >= 20) {
				...
				sgg_execcmd0(0x0080, size0, fbuf->paddr, fp0, 0x0000); /* 슬롯을 사용하지 않는 매핑 */
				for (i = 0; i < 15; i++)
				...
				if (j == 0 && (fp0[0] == 0x82 || fp0[0] == 0x83 || fp0[0] == 0x85 || fp0[0] == 0x89)) {
					...
					if (size0 + size1 <= FILEAREA) {
						if ((i = sgg_execcmd1(2 * 4 + 12, 0x0084, size1, 0, 0x0000)) ! = -1) { /* 메모리를 받는다 */
							sgg_execcmd0(0x0080, size1, i, fp1, 0x0000); /* 슬롯을 사용하지 않는 매핑 */
							if (fp0[0] == 0x82)
		...
		if (auto_dearc) {
			...
			size0 = fbuf->size;
			if (size0 < FILEAREA) {
				sgg_execcmd0(0x0080, FILEAREA, fbuf->paddr, fp0, 0x0000); /* 슬롯을 사용하지 않는 매핑 */
			...
			sar:
					...
					if ((k & ~1) == 0) {
						for (i = 0; i < 5; i++)
							gets7s(&p);
						align = (1 << gets7s(&p)) - 1;
						if (align >= 0xfff) { /* 4KB이상의 align이 필요 */
							j = gets7s(&p);
							...
							if (j == 0 && (q[0x0b5] & 0x01) ! = 0) { /* 최초의 엔트리가 반드시 주파일 */
								...
								if (abuf) {
									...
									if (abuf) {
										...
										abuf->align = align;
										/* 여기서 아카이브(archive)내의 모든 파일에 대해서 osarjc한다 */
										while ((i = gets7s(&p)) ! = 0) {
											p += i; /* 파일명 스킵 */
											i = gets7s(&p); /* 속성 */
											if ((i & 0x20) == 0)
				...
				if (size0 >= 0x00050000 + 29 && fp0[0x00] ! = 0xcc && fp0[0x0b] == 0x00 && fp0[0x0c] == 0x02 && (*(int *) &fp0[0x40]) == 0x36314653) {
					...
					if (j == 0 && (fp0[0x000500b5] & 0x01) ! = 0) { /* 최초의 엔트리가 반드시 주파일 */
						abuf = arcsub_srchdslt(fbuf->dirslot);
						if (abuf) {
							abuf->linkcount++;
							sgg_execcmd0(0x0020, 0x80000000 + 5, 0x1244, 0x0134, -1, fbuf->size, fbuf->paddr, 0x0000);
...
void jsub_create_task0()
/* pjob->bank만 참조 */
/* 에러시에는 bank->tss가 0이 되어 있으므로, 필요하면 -1을 대입하는 것 */
{
	struct STR_JOBLIST *pjob = &job;
...
int jsub_create_task1(int *sbp)
{
	...
	if (bank->tss = tss = sbp[1]) {
		...
		sgg_settasklocallevel(tss,
			0 * 32 /* local level 0 (sleeve 레벨) */,
			27 * 64 + 0x0100 /* global level 27 (sleeve) */,
			-1 /* Inner level */
		);
		bank->Llv[0]. global = 27;
		bank->Llv[0]. inner  |= -1;
		sgg_settasklocallevel(tss,
			1 * 32 /* local level 1 (기동·시스템 처리 레벨) */,
			12 * 64 + 0x0100 /* global level 12 (일반 어플리케이션) */,
			i = defaultIL /* Inner level */
		);
		bank->Llv[1]. global = 12;
		bank->Llv[1]. inner  = i;
		sgg_settasklocallevel(tss,
			2 * 32 /* local level 2 (통상 처리 레벨) */,
			12 * 64 + 0x0100 /* global level 12 (일반 어플리케이션) */,
			i /* Inner level */
		);
...
void job_view_file1(int cond)
{
	...
	//	pjob->bank->tss = 0; /* 개방 */
		...
		if (fbuf == NULL)
			goto error;
		fbuf->size = 13;
		if ((fbuf->paddr = sgg_execcmd1(2 * 4 + 12, 0x0084, fbuf->size, 0, 0x0000)) == -1) /* 메모리를 받는다 */
			goto error;
		fbuf->linkcount = 1;
		fbuf->dirslot = -1;
		fbuf->readonly = 0;
		...
		sgg_directwrite(0, 4, 0, 0x0200, /* 슬롯 0x0200에 붙인다 */
			(0x003c /* slot_sel */ | pjob->bank->tss << 8) + 0xf80000, (int) &fbuf->virtualmodule, 0x000c);

		sgg_execcmd0(0x0080, 13, fbuf->paddr, fp0, 0x0000); /* 슬롯을 사용하지 않는 매핑 */
		for (i = 0; i < 14; i++)
			fp0[i] = "This is test. \n"[i];
...
err:
		*pfmode = STATUS_MAKE_PLAIN_BOOT_DISK;
		...
		pjob->param[0] = (int) fbuf; /* 캐쉬 히트일지도 모르기 때문에 갱신 */
...
void job_create_sysdisk1(int cond)
{
	...
	//	pjob->fbuf->linkcount = 0; /* 개방 */
		...
		*pfmode = STATUS_MAKE_PLAIN_BOOT_DISK;
		...
		pjob->param[1] = (int) pjob->fbuf; /* 캐쉬 히트일지도 모르기 때문에 갱신 */
		*pfmode = STATUS_LOAD_BOOT_SECTOR_CODE_COMPLETE; /* 'S'와'Enter'와'F'와'R' 밖에 입력할 수 없다 */
	...
	//	pjob->fbuf = 0; /* 개방 */
		...
		*fp0 = 0xcc; /* 개서 */
		...
		sgg_execcmd0(0x0020, 0x80000000 + 10, 0x1249, 0x0150,
			...
			0x4243 /* to pokon0 */, 0x7f000002, SIGNAL_REFRESH_FLIST0, 1 /* 정상 종료가 -1이니까 */, 0x0000);
	}
	return;
}
...
void job_resize_sub2(int cond)
{
	...
	if (cond == 0) {
		/* 로드 실패 */
	//	pjob->fbuf = 0; /* 개방 */
		/* 어플리케이션으로부터는 아니기 때문에 시그널은 보내지 않는다 */
		pjob->now = 0;
	} else {
...
void job_exec_psf_sub0(int cond)
{
	...
	if (cond) {
		...
		sgg_execcmd0(0x0080, FILEAREA, pjob->fbuf->paddr, fp, 0x0000); /* 슬롯을 사용하지 않는 매핑 */
		while (fp < fp1) {
			p = cmdline0;
			while (fp < fp1 && *fp <= ' ')
				fp++;
...
void list_set(struct FILESELWIN *win)
{
	...
	// 어플리케이션이 LIST_HEIGHT개 이하 때를 위한 처치
	for (i = 0; i <= LIST_HEIGHT; i++)
		list[i]. name[0] = '\0';

	// 확장자(extension) ext의 것만을 추출해 list에
	fp = file + 1; // 최초의 요소는 더미
	lp = list;
...
void openselwin(struct FILESELWIN *win, const char *title, const char *subtitle)
{
	...
	lib_definesignal1p0(0, 0x0200, 0x1010 /* 감도 1, 창내 좌표 */, &win->window, win->sigbase+SIGNAL_MOSPOS);
	lib_definesignal1p0(0, 0x0200, 0x1020 /* 감도 1, press, left */, &win->window, win->sigbase+SIGNAL_MOSBTN);
	lib_definesignal1p0(0, 0x0200, 0x1030 /* 감도 1, release, left */, &win->window, win->sigbase+SIGNAL_MOSBTN+3);
...
void arcsub_map(struct STR_ARCBUF *arc, unsigned char *fp0)
{
	...
	sgg_execcmd0(0x0080, arc->size, arc->paddr, fp0, 0x0000); /* 슬롯을 사용하지 않는 매핑 */
	return;
}
...
void OsaskMain()
{
	...
	*(pjob->rp = pjob->wp = pjob->list) = 0; /* 쌓인 일은 없다 */
...
//	pcons->buf = (char *) malloc((CONSOLESIZEX + 2) * (CONSOLESIZEY + 1));
	...
	bootflags_check:
				if (bootflags == 0x03) {
					...
					/* 여기서"OSASK0.PSF"배치파일을 찾아, read&실행 */
					/* Pokon Script File */
					/* 줄머리가'/'라면 주석으로 간주한다 */
					/* 파일 영역의 마지막 4096바이트를 사용 */
					/* 들어갈 수가 없으면 에러이므로 실행하지 않는다 */
			...
			case SIGNAL_TERMINATED_TASK:
				...
				/* 태스크 tss에 통지하려고 생각하고 있던 다이얼로그를 모두 클로우즈 */
				...
				/* 본래는, 태스크를 종료하기 전에 pokon0에 통지가 있어야 함 */
				/* 그렇지 않으면 종료했는데 시그널을 보내 버리는 일이 일어날 수 있다 */
				/* 슬롯의 클로우즈를 검출하면 통지는 가능 */
...
//lib_putstring_ASCII(0x0000, 0, 0, &win[0]. subtitle.tbox, 0, 0, "debug! (2)");
			...
			case SIGNAL_REQUEST_DIALOG2:
				/* 우선 버퍼에 넣어 둔다 */
				siglen = 6;
				for (swait = selwait; swait->task; swait++); /* 지나치는 일은 생각하지 않았다 */
				selwaitcount++;
				swait->task    = sbp[1];
				swait->slot    = sbp[2];
				swait->bytes   = sbp[3];
...
/* lib_putstring_ASCII(0x0000, 0, 0, &win[0]. subtitle.tbox, 0, 0, "debug! debug! "); */
				...
				if (fbuf->pipe) {
					...
					if (((sbp[2] + 0xfff) & 0xfffff000) ! = ((fbuf->size + 0xfff) & 0xfffff000))
						goto resize_error; /* 페이지수가 바뀌는 것 같으면 에러 */
					sgg_execcmd0(0x0074, 0, fbuf->virtualmodule, 0x0000); /* virtualmodule의 파기 */
					fbuf->size = sbp[2];
					fbuf->virtualmodule = sgg_createvirtualmodule(fbuf->size, fbuf->paddr);
				...
				if (pjob->free >= 8 + 5 + 4) {
					/* 빈 곳이 충분히 있다 */
					/* 이 이후는 상당한 부실이 있다(우선 움직이면 좋다고 하는 레벨) */
					/* 디스크가 교환될지도 모르는 것에 배려하고 있지 않다 */
					/*   →아니, 분명하게 배려하고 있다 */
	...
	resize_find:
						...
						selwincount++;
						win->lp = win->list; /* window-close 처리중이 아닌 것을 명시 */
						win->task = sbp[3];
						win->mdlslot = sbp[5];
						win->siglen = 2;
						win->sig[1] = sbp[4];
						/* reload를 위해서(때문에), vmref를 버린다 */
						for (vmr = vmref; vmr->fbuf ! = fbuf || vmr->task == 0; vmr++);
						vmr->task &= 0;
						unlinkfbuf(fbuf);
						fbuf->linkcount |= -1; /* 이 fbuf를 리저브 한다 */
	...
	resize_error:
		...
		no_wb_cache2:
			...
			case SIGNAL_CHECK_WB_CACHE_NEXT:
				...
				if (check_wb_cache(fbuf) == NULL) /* 종료하면 NULL를 돌려준다 */
					goto job_end;
				break;
	...
	job_end:
			...
			case SIGNAL_FORMAT_COMPLETE: /* 포맷 완료 */
			//	siglen = 1;
write_exe:
				...
				sgg_format2(0x0128 /* 1,440 KB포맷용 비압축 */,
					//	0x011c /* 1,760 KB포맷용 비압축 */
					//	0x0128 /* 1,440 KB포맷용 비압축 */
					//	0x0138 /* 1,440 KB포맷용 압축 */
					pjob->fbuf->size, pjob->fbuf->paddr, fbuf->size, fbuf->paddr,
					SIGNAL_WRITE_KERNEL_COMPLETE /* finish signal */); // store system image
				break;

			case SIGNAL_WRITE_KERNEL_COMPLETE: /* .EXE write 완료 */
			//	siglen = 1;
				unlinkfbuf((struct FILEBUF *) pjob->param[0]);
			...
			case 3: /* launcher 기능(run) 일부러 .PSF를 만들지 않아도 괜찮고 만들어도 괜찮다 */
				siglen = 4; /* sbp[1~3]은"FILENAMEEXT\0" */
			...
			/* selwin[0]에 대한 특별 커멘드 */
		//	siglen = 1;
			if (fmode >= STATUS_FORMAT_COMPLETE)
				goto nextsig; /* boot중은 무시 */
		...
		/* 포맷 모드에 들어가려면 다른 파일 셀렉터가 모두 닫혀져 있지 않으면 안된다 */
		/* 또, 포맷 모드에 들어가 있는 동안은 파일 셀렉터가 열리지 않는다 */
		...
		//	siglen = 1;
			if (CONSOLE_KEY_SIGNAL_START <= sig && sig <= CONSOLE_KEY_SIGNAL_END) {
				...
				switch (sig) {
				case CONSOLE_VRAM_ACCESS_ENABLE /* VRAM 액세스 허가 */:
					break;

				case CONSOLE_VRAM_ACCESS_DISABLE /* VRAM 액세스 금지 */:
					lib_controlwindow(0x0100, &pcons->win);
					break;

				case CONSOLE_REDRAW_0:
					/* 재묘화 */
					lib_controlwindow(0x0207, &pcons->win); /* 콘솔은 테두리만으로 좋으니까 */
				case CONSOLE_REDRAW_1:
					/* 차분재묘화 */
					lib_controlwindow(0x020f, &pcons->win); /* 범위만＆차이분 모드 */
					break;
				...
				case CONSOLE_INPUT_ENTER /* console에의 Enter 입력 */:
					if (pcons->curx < 0)
						break;
			...
			prompt:
				...
				case CONSOLE_INPUT_BACKSPACE /* console에의 BackSpace 입력 */:
					if (pcons->curx >= 0) {
						if (pcons->cursorflag ! = 0 && pcons->cursoractive ! = 0) {
							pcons->cursorflag = 0;
							putcursor();
		...
		//	siglen = 1;
			if (fmode == STATUS_LOAD_BOOT_SECTOR_CODE_COMPLETE && sig == SIGNAL_ENTER) {
				...
				for (i = 0; i < MAX_FILEBUF; i++)
					fbuf0[i]. dirslot = -1;
				sgg_format(0x0124 /* 1,440 KB포맷 */, SIGNAL_FORMAT_COMPLETE /* finish signal */); // format
				/* 1,760 KB포맷과 1,440 KB포맷의 혼재 모드는 0x0118 */
				fmode = STATUS_FORMAT_COMPLETE;
				goto nextsig;
			}
...
listup:
				...
				//	if (cur < 0 /* 파일이 1개도 없다 */)
				//		break;
				//	파일이 없는 경우 cur == -1 && lp[0].name[0] == '\0'
				//	그래서, 이하의 if가 성립하지 않는다.
					if (lp[cur + 1]. name[0]) {
				...
				page_down:
					...
					if (0 >= i)
						break;
					if (i > LIST_HEIGHT) // 나머지가 1화면분 보다 많다
						i = LIST_HEIGHT;
					lp += i;
					cur -= i;
					if (cur < 0)
				...
				case SIGNAL_TOP_OF_LIST:
					if (cur < 0 /* 파일이 1개도 없다 */ || lp - list + cur <= 0)
						break;
					lp = list;
					cur = 0;
					goto listup;
...
#if 0	/* 일반 윈도우가 닫혀지지 않아 삭제 */
					if (i == 0){
						lib_execcmd0(0x48, 1, &win->window, 0xc0, 0,0, 15,0, 2, 'C', 'R', 0);	/* 디버그 메세지 */
						break;
					}
#endif
					/* 캔슬을 통지해서 닫는다 */
					/* 대기하고 있는 것이 있으면 응한다(응하는 것은 127을 받고 난 후로 한다) */
...
nextsig:
		...
		while (*sbp == 0 && selwaitcount ! = 0 && selwincount < MAX_SELECTOR && fmode == STATUS_RUN_APPLICATION) {
			...
			selwaitcount--;
			selwincount++;
			/* task, slot, num, siglen, sig[16]을 취득 */
			win->lp = win->list; /* window-close 처리중이 아닌 것을 명시 */
			win->task = swait->task;
		...
		//	i = swait->bytes & 0x80000000;
			...
			if (subcmd[0] == 0xffffff01 /* num */) {
				...
				if (bank ! = NULL && bank->arcbuf ! = NULL) {
					...
					if (p) {
						i = (arcsub_gethex(&p[0x30]) >> 4) & 0x07; /* options의 bit4-6 */
						if ((i == 1 && fp == NULL) || i == 0) {
							/* 반응한다 */
							int size3;
			arcmatch:
			...
			if (subcmd[0] == 0xffffff03 /* direct-name */) {
				/* 우선, 이름은 12바이트 고정(마지막 바이트는 00) */
				/* 마지막 바이트를 01로 해도 괜찮다. 01은 강제 이동 */
				win->siglen = subcmd[6];
				win->sig[0] = subcmd[7];
				win->sig[1] = subcmd[8];
				...
				if (bank ! = NULL && bank->arcbuf ! = NULL) {
					...
					if (p) {
						i = arcsub_gethex(&p[0x30]) & 0x07; /* options의 bit0-2 */
						if ((i == 1 && fp == NULL) || i == 0) {
				...
				if (fp == NULL) {
					...
					if ((swait->bytes & 0x80000000) ! = 0 && pjob->free >= 16 && (fbuf = searchfrefbuf()) ! = NULL) {
						...
						writejob_n(2, JOB_SEARCH_FP, (int) win); /* fp__에 대입한다 */
						writejob_3p(&subcmd[2]);
...
error_sig:
				/* 에러이므로 slot를 무효로 할까하고도 생각했지만, 거기까지 할 것도 없음 */
#if 0
...
void open_console()
/*	콘솔을 오픈한다 */
/*	커서 점멸을 위해서 setmode를 줍는다 */
/*	커서 점멸을 위한 타이머를 enable로 한다 */
{
...
find:
	...
	sgg_settasklocallevel(task,
		2 * 32 /* local level 2 (통상 처리 레벨) */,
		16 * 64 /* global level 16 (일반 어플리케이션) */,
		l /* Inner level */
	);
	bank->Llv[2]. global = 12;
	bank->Llv[2]. inner  = l;
...
error:
	...
	fbuf2->size = 1;
	if ((fbuf2->paddr = sgg_execcmd1(2 * 4 + 12, 0x0084, fbuf2->size, 0, 0x0000)) == -1) /* 메모리를 받는다 */
		goto error;
	fbuf2->linkcount = 2;
	fbuf2->dirslot = -1;
	fbuf2->readonly = 0;
...
//	sgg_execcmd0(0x0080, 14, fbuf2->paddr, fp0, 0x0000); /* 슬롯을 사용하지 않는 매핑 */
//	for (i = 0; i < 14; i++)
//		fp0[i] = "This is test. \n"[i];
...
/*	"console0.bin"를 준비하고, slot210에 hw.txt를 접속해 시그널을 보내 준다 */
/* 어떤 시그널?  */
/*	사이즈 변경 확정, 리드 확정 */
/*	console는 이것에 응해, lock을 걸어 읽어들인다 */
/*	일이 끝나면 sleeve해서 기다린다. 유저가 태스크 종료 시그널을 보내 끝 */
/*	쉘과 통신하고 싶으면, 그러한 connection가 필요...별로 없어도 괜찮은가. shellcall 하면 어차피 안다 */


/*	자동 신장형의 확정 범위 관리는, srml로 완성된다. 확정 영역을 더해 가 선두가 0으로 사이즈가 성장하면 연락 */
/*	이것은 너무나 전형적이어서, 특별한 모드를 만든다(read only·write only) */
/*	사이즈 확정과·라이트 확정(게다가 순서 방향) 밖에 통지하지 않는다 */
/*	32 bit 모드, 64 bit 모드, 128 bit 모드, 리저브가 있다 */
/*  sighed, 확정 후의 사이즈(32bit) */
/*	sighed, 사이즈, 확정 영역 */

/* 통지는 여러 가지 있다. 확정인가 일시인가. 사이즈, 라이트 완료, 리드 완료, 옵셔널 */
/* 액세스 lock 요구(리드, 라이트, 사이즈 변경권) */


/* 모듈 시그널 허브 */

/*
(1) 액세스 락 요구 (리드 한때·리드 확정·리드 라이트)
(2) lock 개방
(3) 액세스 가능 범위 통지 (리드·라이트, 일시·확정)
(4) 통지 시그널 설정
(5) 통지 시그널 송신 (리드·라이트·사이즈 변경, 일시·확정)
*/
