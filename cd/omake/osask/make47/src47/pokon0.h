...
enum {
	...
	FMODE5,
	FMODE6,
	STATUS_LOAD_BOOT_SECTOR_CODE_COMPLETE,/* 'S'와'Enter'와'F'와'R' 밖에 입력할 수 없다 */
...
#define SIGNAL_BOOT_COMPLETE                    99 /* boot 완료 */
#define SIGNAL_TERMINATED_TASK                  0x0080 /* terminated task */
#define SIGNAL_REQUEST_DIALOG                   0x0084 /* 다이얼로그 요구 시그널 */
#define SIGNAL_REQUEST_DIALOG2                  0x0088
#define	SIGNAL_FREE_FILES						0x008c /* 파일 개방 요구 */
#define SIGNAL_RESIZE_FILE						0x008d /* cmd, addr, new-size, task, sig, slot */
#define	SIGNAL_NEED_WB							0x0090 /* 파일 캐쉬는 write back가 필요 */
#define	SIGNAL_NO_WB_CACHE						0x0094 /* to clear need_wb */
#define	SIGNAL_CHECK_WB_CACHE_NEXT				0x0098 /* JOB_CHECK_WB_CACHE의 작업용 시그널 */
#define	SIGNAL_REFRESH_FLIST					0x009c
#define	SIGNAL_REFRESH_FLIST0					0x009d /* 파일 리스트는 변경하지 않는다 */
#define SIGNAL_RELOAD_FAT_COMPLETE              0x00a0 /* FAT 재독 보고 포함 완료(Insert) */
#define	SIGNAL_JSUB								0x00a4 /* 범용 시그널 */
#define SIGNAL_FORMAT_COMPLETE                  0x00b8 /* 포맷 완료 */
#define SIGNAL_WRITE_KERNEL_COMPLETE            0x00bc /* .EXE write 완료 */
#define	SIGNAL_RESIZE_SUB0						0x00bd /* size == 0 으로부터의 resize */
#define SIGNAL_VBELIST							0x00be /* VESA 정보 주소 통지(AT만, winman0로부터) */
...
enum {
	...
	SIGNAL_DISK_CHANGE0, /* 10개 필요(22-31) */
...
struct FILEBUF {
	...
	char readonly, pipe /* resize시의 거동을 결정한다 */;
};

struct STR_BANK { /* 84bytes */
	int tss; /* 여기가 0이라면 미사용 bank, 부의 값이라면 예약 bank */
	struct FILEBUF *fbuf;
	struct STR_ARCBUF *arcbuf;
	char name[12];
	struct {
...
//	subtitle_str[0] == '\0' : 윈도우 클로우즈 완료
//	task == 0 :	미사용 마크(subtitle_str[0]과 같지 않은가? )
//	lp == NULL : close 처리중
//	mdlslot == -1 : 태스크 종료에 의해 캔슬되었다
//	mdlslot == -2 : close 처리중

	/*
		close window를 실시하면, lp = NULL가 된다.
		closed를 받으면, lp = list가 된다.
		로드가 끝나면, mdlslot가 ―2가 된다.
	*/
...
struct STR_PROCESS_TASK {
	...
	char flags; /* bit0:1이라면 이 태스크가 죽을 때 프로세스를 연루한다 */
};
