...
struct STR_BIM2BIN {
	...
	UCHAR *outname; /* '\0'로 끝나는, work의 어딘가에의 포인터 */
...
int bim2bin_main(struct STR_BIM2BIN *params)
{
	...
	GO_stdout.p1 = params->err0; /* stdout는 없음 */
	GO_stdout.dummy = ~0;
...
/* distance의 증가량이 l증가량의 p배를 넘고 있는 것 같으면, 손해이므로 채용하지 않는다 */
...
const int lzcompress_tek0(int prm, unsigned char *buf, int k, int i, int outlimit, int maxdis)
{
	...
	*subbuf = 0x00;
	...
	for (z = 0; z < 16; z++) {
		for (ii = 0; ii < 2; ii++) {
			for (j = 0; j < 4; j++) {
				/* 최초로 dis_s를 산출해서 l1b를 정확하게 계산한다 */
