...
struct STR_CC1MAIN {
	...
	UCHAR *outname; /* '\0'로 끝난다, work의 어딘가의 포인터 */
...
struct STR_NASKMAIN {
	UCHAR *cmdlin; /* '\0'로 끝난다 */
	UCHAR *outname; /* '\0'로 끝난다, work의 어딘가의 포인터 */
	UCHAR *listname; /* '\0'로 끝난다, work의 어딘가의 포인터 */
	UCHAR *dest0, *dest1; /* 출력 파일(dest0는 고쳐 쓸 수 있다) */
	UCHAR *list0, *list1; /* 출력 파일(list0는 고쳐 쓸 수 있다) */
	UCHAR *err0, *err1; /* 콘솔 메세지(err0는 고쳐 쓸 수 있다) */
	UCHAR *work0, *work1;
	int errcode;
};
...
struct STR_OBJ2BIM {
	UCHAR *cmdlin; /* '\0'로 끝난다 */
	UCHAR *outname; /* '\0'로 끝난다, work의 어딘가의 포인터 */
	UCHAR *mapname; /* '\0'로 끝난다, work의 어딘가의 포인터 */
	UCHAR *dest0, *dest1; /* 출력 파일(dest0는 고쳐 쓸 수 있다) */
...
struct STR_SJISCONV { /* STR_GAS2NASK와 콘파치 */
	UCHAR *cmdlin; /* '\0'로 끝난다 */
...
void OsaskMain()
{
	...
	/* 라이브러리의 초기화(반드시 최초로 하지 않으면 안 된다) */
	...
	/* 시그널 정의 */
	lib_definesignal1p0(0x7e - 0x20, 0x0100, 0x20, window, 128 + 0x20); /* ASCII 캐릭터 입력 */
	lib_definesignal1p0(0, 0x0100, 0xa0, window, 128 + '\n'); /* Enter 입력 */
	lib_definesignal1p0(0, 0x0100, 0xa1, window, 128 + '\b'); /* Backspace 입력 */
...
int execute(UCHAR *p, struct CONSOLE *cons)
{
	...
	if (p[0] == 'c' && p[1] == 'c' && p[2] == '1' && p[3] <= ' ') {
		...
		*(str_cc1main.err0) = '\0';
		...
		free(s); /* outname이 있으므로 이 타이밍에서 free */
		free(q);
put_termcode:
		if (1 <= i && i <= 6)
...
int osaopen(UCHAR *f, int slot, int opt)
{
	...
	setosaname(f);
	lib_initmodulehandle0(0x0008, slot); /* user-dir 초기화 */
...
skip:
	...
	lib_unmapmodule(0, 512 * 1024, lib_readCSd(0x0010));
	lib_initmodulehandle0(0x0008, 0x220); /* user-dir에 초기화 */
...
/* 0이 반환되면 시그널 없음 */
{
	...
	if (*sig_ptr == REWIND_CODE) {
		/* REWIND 시그널을 받았다 */
		/* 직후 값의 분만큼 시그널을 처리했던 것으로 해서 포인터를 선두로 되돌린다 */
		lib_waitsignal(0x0000, *(sig_ptr + 1), 0);
		sig_ptr = signalbox0;
	}
	signal = *sig_ptr++;
	/* 1 시그널 받은 것을 라이브러리에 통지 */
...
/* 여기에서 아래에 별도의 소스로 나누어 라이브러리화하는 것이 좋을 것이다.  */
/* 그러나 분할 컨바일에 대해 설명하는 것이 귀찮아서,
	여기에서는 나누지 않고 작성했다.  */

#define	CONS_IBUFSIZ	1024
#define	CONS_EBUFSIZ	  64

#if 0 /* lmt가 2로 하지 않아도 사용할 수 있는 매크로 */
...
const int cons_keyin(struct CONSOLE *cons, int c)
/* 콘솔에 한 글자 입력 */
/* 입력이 버퍼에 들어가지 않을 경우에는 0이 아닌 값을 돌려준다 */
{
	if (cons->curflag) {
		/* 커서 표시중 */
...
const int cons_readycgetc(struct CONSOLE *cons)
/* input_buf가 비어있지 않으면 0영이 아닌 값을 돌려준다 */
{
	return cons->ibuf_wptr ! = cons->ibuf_rptr;
}

const int cons_readycgets(struct CONSOLE *cons)
/* inbut_buf에 '\n'이 포함되어 있으면 0이 아닌 값을 돌려준다 */
{
