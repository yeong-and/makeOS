...
void	Generator::OpenSegment(SegmentList* segment){
	...
	switch(segment->combine){
	  case TK_PRIVATE:                          break;	// private는 지정 없음
...
void	Generator::Op1(Token command, Parameter& param){
	...
	switch(command){
	  case TK_INC:
		if(param.pdepth ! = 0 && param.ptype == TK_FAR){			// 이상한 추가분
...
void	Generator::RegistBecome(Parameter& param1, Parameter& param2){
	...
	if(param1.size == param2.size || param1.size == 0 || param2.size == 0){
		...
		if(param2.paramtype == P_MEM){
			param2.size = param1.size;		// Param2LPSTR()에서는 type은 보지 않기 때문에 size만 바꾸면 된다
		}else if(param2.paramtype == P_REG){
			if(RegisterDown(param1, param2) ! = 0) return;	// 에러라면 처리하지 않고 종료
		}else{
...
void	Generator::RegistAdd(Parameter& param1, Parameter& param2){
	...
	if(param1.size == param2.size || param1.size == 0 || param2.size == 0){
		if((param1.paramtype == P_REG && param1.base->type == R_GENERAL || param1.paramtype == P_MEM)
			...
			if(param1.pdepth ! = 0 && param1.ptype == TK_FAR){			// 이상한 추가분
...
// 인수를 서포트하면 인수를 로컬 변수에 추가한다
// 로컬 변수를 서포트하면 스택의 조작도 한다
void	Generator::StartFunction(LPSTR funcname, Token ptype, SegmentList* segment){
	...
	if(prevseg == NULL){
		...
		CloseSegment(prevseg);
		OutputMASM("", "", "", "");		// segment가 계속되고 있기 때문에 적어도 개행을 넣는다
		OpenSegment(segment);
	}else{
		OutputMASM("", "", "", "");		// proc가 계속되고 있기 때문에 적어도 개행을 넣는다
	}
	switch(ptype){
...
// 배열의 초기화는 initialize에 들어가 있는 것을 그대로 썼을 뿐의 사양.
// 그 중 어떻게든 할 예정.
...
void	Generator::StartSegment(SegmentList* segment){
	...
	if(prevseg ! = segment){
		...
		OutputMASM("", "", "", "");		// 정의가 계속되고 있기 때문에 적어도 개행을 넣는다
	}
}
