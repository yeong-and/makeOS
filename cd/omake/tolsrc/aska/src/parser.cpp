...
HRESULT	Parser::Sizeof(Parameter& param){
	...
	scanner.GetToken();
//	if(scanner.PeekToken() == TK_STRUCT) scanner.GetToken(); // 「struct 태그명」을 허가하기 위한 고육
	...
	switch(scanner.PeekToken()){
	  case TK_NEAR:
		...
		scanner.GetToken();						// 다음으로 진행한다
		if(scanner.GetToken() ! = TK_MUL){		// 지금은 *가 얼마든지 계속된다
	  ...
	  case TK_FAR:
		...
		scanner.GetToken();						// 다음으로 진행한다
		if(scanner.GetToken() ! = TK_MUL){		// 지금은 *이 얼마든지 계속된다
...
HRESULT	Parser::Address(Parameter& param){
	LabelList* label;
	Parameter	selparam;			// Selector()는 param가 파괴되기 때문에
...
HRESULT	Parser::Selector2(Parameter& param){
	...
	switch(scanner.PeekToken()){
	  case TK_LSQ:
		...
		if(Selector2(param) ! = 0) return 1;		//재귀
		break;
	  case TK_DOT:
...
// offset 캐스트나 segment 캐스트는 지정할 수 없다
HRESULT	Parser::Array(Parameter& param){
	Token			ope = TK_PLUS;
	int				PRnest, SQnest;
	Parameter		selparam, castparam;		// Selector()는 param가 파괴되기 때문에
	SegmentList*	seg;
	RegisterList*	reg;
	LabelList*		label;
	switch(scanner.GetToken()){
	  case TK_LPR: PRnest=1; SQnest=100; break;	// 아마 관계없으면 0이 되지 않는다
	  case TK_LSQ: SQnest=1; PRnest=100; break;	// 아마 관계없으면 0이 되지 않는다
	  default: return 0;
	}
	param.paramtype = P_MEM;
	...
	while(true){
		switch(scanner.GetToken()){
		  ...
		  case TK_PLUS:  param.disp+="+";                      castparam.type=NULL; continue;	// 단항 연산자+
		  case TK_MINUS: param.disp+="-";     ope=TK_MINUS;    castparam.type=NULL; continue;	// 단항 연산자-
		  case TK_CPL:   param.disp+=" NOT "; ope=TK_CPL;      castparam.type=NULL; continue;	// 단항 연산자~
		  case TK_LPR:
	...
	if(param.type == NULL){
		...
		if(param.pdepth > 0) param.pdepth--;	// 임시. 진짜는 if절만으로 됨
	}
	return 0;
}
	  ...
	  case TK_LABEL:
			...
			switch(reg->type){
			  case R_SEGREG:
				...
				if(scanner.PeekToken() ! = TK_COLON){
					...
					PointerCheck(reg);
					if(reg->bBase == false){
						Error("Must be Seg:Base");	// segreg:const는 쓸 수 없다
						return 5;
					}
		...
		if(label ! = NULL){
			...
			if(castparam.type ! = NULL){
				...
				param.ptype=castparam.ptype; param.size=castparam.size;
				param.pdepth=castparam.pdepth; param.paramtype=castparam.paramtype; // Cast에 의해 추가
				//!  offset이 수치인 앨리어스(alias)를 동작시키기 위해 이상한 추가
				if(param.base == NULL) param.paramtype = P_IMM;
			}
			break;
		}
		// 변수도 레지스터도 아닌 경우는 주소 라벨이라고 본다
...
// rep, true, false등의 커맨드 캐스트는 어떻게 건네주는지?
// (의 뒤 토큰부터 건네준다
...
HRESULT	Parser::Immedeate(Parameter& param){
	...
	while(bFlag == false){
		switch(scanner.GetToken()){
		  ...
		  case TK_PLUS:  param.disp+="+";                       continue;	// 단항 연산자+
		  case TK_MINUS: param.disp+="-";                       continue;	// 단항 연산자-
		  case TK_CPL:   param.disp+=" NOT ";                   continue;	// 단항 연산자~
		  case TK_LABEL:
			label = generator.FindLabelList(scanner.GetLabel());
...
void	Parser::Expression(void){
	...
	switch(command){
	  ...
	  case TK_CPLA: case TK_NEG:
		generator.Op1(command, param1);		// 사실은 param1==param2의 체크를 한다
		break;
	  default:
		  ...
		  case TK_LABEL:
			...
			if(str[0] == '@'){
				...
				if(generator.FindLocalLabelList(str) ! = NULL){
					...
					// 라벨의 전방 참조는 할 수 없기 때문에 로컬 라벨이라고 가정
...
void	Parser::DefineFunction(void){
	...
	// 함수의 선언을 정의한다. StatementLevel에 주의
	if(StatementLevel == 2){
	...
	if(scanner.PeekToken() == TK_DLM){
		...
		generator.AddGlobalLabelList(str, label);	// bAlias=true는 선언을 의미
		return;
	}
	// 여기서부터 함수 자체를 정의한다
	...
	if(label ! = NULL){
		...
		label->bAlias = false;		// 실체가 정의되었으므로 bAlias=false로 한다
	}else{
		label = new LabelList(false, generator.FindTagList("void")
...
//	}
	...
	while(scanner.PeekToken() ! = TK_DLM){
		...
		if(StatementLevel == 0){
			...
			generator.AddLocalLabelList(str, label);	// 로컬 변수의 경우는 이것으로 끝
		}
		switch(scanner.PeekToken()){
		  case TK_COMMA:
			scanner.GetToken();
...
// 현재는 구조체는 물론, 배열의 초기화도 서포트하고 있지 않다(어떻게든 하고 싶다)
// 배열을 서포트하기 위해서는 요소 수를 확실히 파악하여 ', '를 넣을 필요가 있다
...
void	Parser::DefineStruct(void){
	TagList*	tag;
	LPSTR			name;
//	if(scanner.PeekToken() == TK_STRUCT) scanner.GetToken();	// struct 태그명 관련 수정 from Statement()
...
void	Parser::DefineMember(TagList* tag){
	...
	token = scanner.GetToken();
//	if(scanner.PeekToken() == TK_STRUCT) scanner.GetToken(); // 「struct 태그명」을 허가하기 위한 방법
	switch(token){
...
void	Parser::DefineSegment(void){
	...
	name = strdup(scanner.GetLabel());
// 모두 같은 파라미터라면 복수사용을 허용하고 있지만, 귀찮아서 체크를 없앰
...
void	Parser::IfStatement(void){
	...
	LocalLabelCounter += 2;		// else와 if문의 마지막 2개부분 확보
	...
	scanner.GetToken();		// 다음으로 진행한다
	generator.Jump(C_JMP, labelcount+1);		// if문의 마지막에 점프
	sprintf(buf, "LL%04X", labelcount);
	generator.LocalLabel(buf);
	StatementSequence();
...
void	Parser::LoopStatement(void){
	...
	labelcount = LocalLabelCounter;
	LocalLabelCounter += 2;		// loop문의 선두와 loop문의 마지막의 2개부분 확보
	LoopLabel[LoopLabelPoint] = labelcount+1;
	LoopLabelPoint++;
	sprintf(buf, "LL%04X", labelcount);
	generator.LocalLabel(buf);
	StatementSequence();
	generator.Jump(C_JMP, labelcount);			// loop문의 선두로 점프
	sprintf(buf, "LL%04X", labelcount+1);
	generator.LocalLabel(buf);
	LoopLabelPoint--;
}
...
void	Parser::AsmoutStatement(void){
	...
	strcpy(buf, scanner.GetLabel() + 1);	// 선두의 "를 제외한다
	buf[strlen(buf)-1] = '\0';				// 마지막 "을 제외한다
...
void	Parser::ForStatement(void){
	...
	generator.Jump(C_JMP, labelcount);			// loop문의 선두에 점프
...
void	Parser::WhileStatement(void){
	...
	labelcount = LocalLabelCounter;
	LocalLabelCounter += 2;		// while문의 선두와 while문의 마지막의 2개부분 확보
	LoopLabel[LoopLabelPoint] = labelcount+1;
	LoopLabelPoint++;
	sprintf(buf, "LL%04X", labelcount);
	generator.LocalLabel(buf);
	...
	generator.Jump(cmptype, labelcount+1);
	StatementSequence();
	generator.Jump(C_JMP, labelcount);			// while문의 선두에 점프
	sprintf(buf, "LL%04X", labelcount+1);
	generator.LocalLabel(buf);
	LoopLabelPoint--;
}
...
void	Parser::DoStatement(void){
	...
	labelcount = LocalLabelCounter;
	LocalLabelCounter += 2;		// do~while문의 선두와 do~while문의 마지막의 2개부분 확보
	LoopLabel[LoopLabelPoint] = labelcount+1;
	LoopLabelPoint++;
	sprintf(buf, "LL%04X", labelcount);
	generator.LocalLabel(buf);
...
void	Parser::ContinueStatement(CompareType cmptype){
	...
	generator.Jump(cmptype, LoopLabel[nest] - 1);		// -1을 하면 loop의 선두에 점프
}
