...
const UCHAR *cmd_ovrcopy(const char *cmd)
{
	...
	if (optmatch(cmd, "to") == 0)
		errend(11); /* ovrcopy command error */
	ovrwritefile(pathfix(cmd + (2 + 2), 1), 2, -1, 0x04); /* 갱신일만 갱신 */
	return cmd + (*cmd + 2);
}
...
optloop:
	...
	if (optmatch(cmd, "nofrag")) {
		/* 아마 efat의 후에 실행해도 올바르게 판정할 수 있다 */
		if ((flag & 0x04) == 0)
			errend(33);
...
err:
	...
	do {
		if (*p == '\0')
			break;
		if (*p ! = 0xe5 && (p[11] & 0x18) == 0) { /* 디렉토리와 볼륨 레벨은 배제 */
			t = &s[2];
			for (i = 0; i < 8; i++) {
				if (p[i] == ' ')
					break;
...
void readfile(const UCHAR *path, int flags)
/* flags :
	0 x01:애트리뷰트(attribute)  로드
	0 x02:애트리뷰트(attribute)  리셋트
	0 x04:타임 로드
	0 x08:타임 리셋트
*/
{
...
err:
		...
		fputs(path, stderr);
		errend(13);
		return; /* 사실은 이 return은 필요없다. warning를 경고하기 위해서 */
	}
...
void writefile(const UCHAR *path, int alloc0, int asiz, int flags)
/* delete 하고 나서 writefile 하면 클러스터 위치도 다시 확보하게 된다 */
/* flags :
	0 x01:애트리뷰트(attribute)  스토어
	0 x02:퍼미션 스토어
	0 x04:타임 스토어
*/
{
...
retry:
	...
	if (KHBIOS ! = NULL) {
		/* KHBIOS 정보를 사용해 파티션 개시 위치를 탐색 */

	}
	BPB = test_BPB(bpb);
	if (BPB == NULL) {
		/* AT호환기 HDD 이미지라고 가정해, 제 1파티션을 찾는다 */
		if (bpb[0x1c9] == 0x00) {
...
UCHAR *test_BPB(UCHAR *p)
{
	...
	unsigned short *s;
	if (p[0x0b] ! = 0x00)
		goto err; /* 128바이트/섹터는 취급할 수 없기로 한다 */
	if (p[0x0c] == 0x00)
		goto err;
	if (p[0x0c] & (p[0x0c] - 1))
		goto err;
...
const UCHAR *opt_imgout(const char *cmd)
{
	...
	if ((BPB[0x0b] | BPB[0x0c] << 8) ! = len_sec)
		errend(37);
...
/* 제대로 하고 싶으면 개조해 주세요 */
{
	...
	#include <unistd.h>
	#include <utime.h>
	#include <time.h>		/* ANSI-C, time_t와 일시의 변환에 사용한다 */
#endif


void getattrtime(struct sar_attrtime *s, const UCHAR *path)
/* s->attr의 bit0-3,5-6는 보존하는 것 */
{
	#if (defined(SAR_MODE_WIN32))
		int i;
		s->attr &= 0x06f;
		...
		if (stat(path, &buf) == 0) {
			struct tm gm;
			s->permission = buf.st_mode & 0777; /* SUID라든지 일단 지운다 */
			/* ANSI-C */
