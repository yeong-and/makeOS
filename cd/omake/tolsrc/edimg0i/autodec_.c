...
static int tek_lzrestore_tek5(int srcsiz, UCHAR *src, int outsiz, UCHAR *outbuf)
{
	...
	wrksiz = 0x180 * sizeof (UINT32) + (0x840 + (0x300 << (lc + lp))) * sizeof (tek_TPRB); /* 최저 15KB, lc+lp=3이라면 36KB */
	work = malloc(wrksiz);
	if (work == NULL)
		return -1;
...
static int tek_decmain5(int *work, UCHAR *src, int osiz, UCHAR *q, int lc, int pb, int lp, int flags)
{
	...
	*q++ = tek_rdget1(rd, prb->lit, lit0cntmsk, 1, &rd->bm[24]) & 0xff;
	...
	while (pos < osiz) {
		s_pos = pos & m_pos;
		if (tek_rdget1(rd, &prb->pb[s_pos]. st[s]. mch, 0x71, 0, rd->ptbm[s > 0]) ^ stk) { /* 비lz */
			...
			if (tek_rdget1(rd, &prb->st[s]. rep, 0x71, 0, rd->ptbm[13]) ^ stk) { /* len/dis */
				...
				if (j >= 4) {
					...
					rep[0] = (2 | (j & 1)) << k;
					if (j < 14) /* k < 6 */
						rep[0] |= tek_rebit(tek_rdget1(rd, &prb->spdis[j & 1][(1 << k) - 2], k | 0x70, 1, rd->ptbm[10 + (k >= 4)]), k);
					else {
						if (stk == 0) {
							if (k -= 6)
