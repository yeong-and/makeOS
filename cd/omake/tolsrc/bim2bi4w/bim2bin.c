...
#define TEK1_BT_NODESIZ		32	/* 반드시 2의 배수 */

/* 등록하고 싶은 수/16의 노드 1 */
/* 등록하고 싶은 수/256+등록하고 싶은 수/4096+등록하고 싶은 수/65536+...의 노드 1 */
/* 1/256(16/15)=1/240이니까 등록하고 싶은 수/240의 노드 0 */
/* 1MB라고 하면, 노드 1은 65536개, 노드 0은 4400개 */

struct STR_BT_NODE0 { /* 260바이트 */
	int nodes, skiplen;
	void *pkey[TEK1_BT_NODESIZ];
	void *node[TEK1_BT_NODESIZ];
	 /* pkey는 각 노드의 최저치(근처의 노드의 최대치보다 작으면, 최저치 보다 더 작아도 좋다) */
};

struct STR_BT_NODE1 { /* 132바이트 */
	int nodes, skiplen;
	void *pkey[TEK1_BT_NODESIZ];
};
...
int main(int argc, char **argv)
{
	...
	//		compress = 6;
		...
		//	static UCHAR table_clv[10] = { }; /* 10 단계 */
		//	5가 충분, 3이 중간, 1이 디폴트, 0이 가장 약함, 9가 최강(99)
		//	10-99 -> 00-88, 90
			complev = getnum(s + 4);
			if (complev > 9)
	...
	if (compress >= 0) {
		...
		if (compress < 3) {
			...
			UCHAR *work = malloc(i = 257 * 1024 + (bsiz + 272) * 8);
			if (compress == 3) {
				buf[0x35] = 0x83; /* tek1 압축 데이터-전개 */
				j = lzcompress_tek1(datasize, buf + data_begin, SIZEOFOVERBUF - 8, overbuf, i, work, bsiz, 0, opt, prm0, maxdis, submaxdis);
			}
			if (compress == 4) {
				buf[0x35] = 0x85; /* tek2 압축 데이터-전개 */
				j = lzcompress_tek1(datasize, buf + data_begin, SIZEOFOVERBUF - 8, overbuf, i, work, bsiz, 1, opt, prm0, maxdis, submaxdis);
			}
		//	if (compress == 5) {
		//		buf[0x35] = 0x86; /* tek3 압축 데이터-전개 */
		//		j = lzcompress_tek3(datasize, buf + data_begin, SIZEOFOVERBUF - 8, overbuf, i, work, bsiz, 0, opt, prm0, maxdis, submaxdis);
		//	}
		//	if (compress == 6) {
		//		buf[0x35] = 0x87; /* tek3 압축 데이터-전개 */
		//		j = lzcompress_tek3(datasize, buf + data_begin, SIZEOFOVERBUF - 8, overbuf, i, work, bsiz, 1, opt, prm0, maxdis, submaxdis);
		//	}
			if (compress == 7) {
				buf[0x35] = 0x89; /* tek5 압축 데이터-전개 */
	...
	if (v48 == 0) {
		...
		for (i = 0; i < 0x48; i++)
			buf[i] = header0[i];
		if (v48 & 2) {
			/* code_end, datasize의 16바이트 align */
...
const int search0a(int prm0, unsigned char *buf, unsigned char *buf0, const int max0, int *p, const int *table)
{
	...
	/* 액세스 해선 안 되는 주소 : buf + max0 */
	for (s = buf - 1; s >= buf0; s--) {
		if (*s == c) {
			for (l = max; l > 0; l--) { /* 고속화를 위해, 뒤에서부터 비교 */
				if (buf[l] ! = s[l])
					goto nextloop;
			}
			/* 일치한 길이을 산출, 다만 max0가 되면 최후 */
			for (l = max; buf[l] == s[l]; ) {
				if (++l >= max0) {
					*p = s - buf;
					return l;
...
/* distance의 증가량이 l증가량의 p배를 넘고 있으면, 손해이므로 채용하지 않는다 */
...
int search0c(int prm0, UCHAR *buf, UCHAR *buf0, int max0, int *p, const int *table, int trees, struct STR_BTREE *btree, UCHAR **l1table)
{
	...
	t = l1table[*buf];
	if (t == NULL)
		 goto fin; /* 1문자 일치한 것도 이미 어디에도 존재하지 않는다 */
	if (t < buf0) {
		l1table[*buf] = NULL; /* 향후 귀찮게 하지 않기 위해 */
		goto fin; /* 제일 가깝지만 그래도 maxdis보다 먼 것 같기 때문에 이것도 끝 */
	}
	dd = t - buf;
	if (dd >= table[0]) {
		max = 1;
	...
	for (tree = 0; tree < trees; tree++) {
		...
		handle[1] = handle[0];
		search_next(&btree[tree], &handle[1]); /* 근처도 본다 */
		...
		ld = search0b_calcld(d) - search0b_calcld(dd); /* distance에 의해 어느 정도 비트수가 바뀔까 */
		l0 = l - (ld + prm0 - 1) / prm0;
		while ((l - l0) * prm0 - ld > 0)
			l0++;

		if (l0 < 2)
			l0 = 2; /* 최소길이 1은 이미 처리 끝 */
		ld = search0b_calcld(d);
		if ((max - l) * prm0 + ld - ld0 < 0) { /* max보다 길게 될 수 있을까?  */
			if (l > 10) {
				max = l;
				*p = d;
				ld0 = ld;
...
#if 0
	/* 버그에 의한 허위 보고가 없는가 확인 */
//	if (max > max0) { puts("len-max err! "); exit(1); } /* 이것은 언제나 OK */
//	for (l = 0; l < max; l++)
//		if (buf[*p+l] ! = buf[l]) { puts("string mismatch err! "); exit(1); } /* 이것도 언제나 OK */
...
/* distance의 증가량이 l증가량의 p배를 넘고 있으면, 손해이므로 채용하지 않는다 */
/* 이것을 증가량이 아니고 감소량으로부터 역추정 */
/* distance에 근거하여, len는 어디까지 줄일 수 있는지를 추정 */
/* distance의 최소치(l1 때)는 일순간에 낼 수 있다 */

/* 1. 우선 distance의 최소치를 요구한다 */
/* 2. 다음에 distance의 최대치를 요구한다 */
/* 3.  */
...
{
	if (putb_count ! = 8) {
		putb_byte = putb_byte << 1 | 1; /* "1"을 보낸다 */
		if (--putb_count)
...
{
	if (putb_count ! = 8) {
		putb_byte = putb_byte << 1 | 0; /* "0"을 보낸다 */
		if (--putb_count)
			putb_byte <<= putb_count;
		if (putb_ptr < 0)
			putb_buf[putb_ptr] = putb_byte;
...
/* 아마, l2a나 l2b보다 우수하다 */
{
	...
	putnum_l1a(i - 1); /* 3을 2로 한다 */
	return;
}

void putnum_df(int d, unsigned int s)
/* s의 bit가 1이라고, 거기까지 출력한 뒤에, s비트를 출력 */
{
	int len;
	unsigned int i = 1;
//	if (d == 0) { /* 리피트 마크 출력 */
...
/* sxsxsxsxs 형식으로 출력 */
{
	if (i <= 2)
		return 2;
	return calclen_l1a(i - 1); /* 3을 2로 한다 */
}

const int calclen_df(int d, unsigned int s)
/* s의 bit가 1이고, 거기까지 출력한 뒤에 s비트를 출력 */
{
	int len, l = 0;
	unsigned int i = 1;
//	if (d == 0) { /* 리피트 마크 출력 */
		...
		if (j == 0) {
			...
			if (j == 0) {
				...
				if (j == 0) {
					...
					len = j;
					if (j <= 127)
						len = getbc0(j, 1); /* 최초의 bit는 1로 정해져 있기 때문에 */
				}
			}
		}
		distance = -1;
...
/* 하위 bit부터 넣어 가는 방법 */
{
	...
	for (l = 0; l < maxlen; l++) { /* 아래부터 들어갈 수 있다 */
...
/* 상위 bit부터 넣어 가는 방법 */
{
	...
	s = 1 << maxlen;
	t0 = calc_totalbits(bit, s);
	for (l = maxlen - 1; l >= 0; l--) { /* 위부터 들어갈 수 있다 */
		t = calc_totalbits(bit, s | 1 << l);
		if (t0 > t) {
			s |= 1 << l;
			t0 = t;
...
int lzcompress_tek0(int prm, unsigned char *buf, int k, int i, int outlimit, int maxdis)
{
	...
	*subbuf = 0x00;
	...
	/* len의 encode 방식의 자동 선택 */
	i0 = 0x7fffffff;
	for (z = 0; z < 16; z++) {
		for (ii = 0; ii < 2; ii++) {
			for (j = 0; j < 4; j++) {
				/* 최초로 dis_s를 산출해, l1b를 정확하게 계산한다 */
...
int lzcompress_tek3(int srcsiz, UCHAR *src, int outsiz, UCHAR *outbuf, int wrksiz, UCHAR *work, int bsiz, int flags, int opt, int prm, int maxdis, int submaxdis)
/* work는 8바이트 align 해 둘 것, src도 8바이트 align이 바람직하다 */
/* 레벨 1까지 밖에 없기 때문에, 이것이라면 256KB까지 밖에 압축할 수 없다 */
/* 레벨 2도 쓰면 16MB. 이것이라면 나쁘지는 않다 */
/* 레벨 3도 쓰면 1GB. 더욱 더 좋다(bim2bin로서는 이것이 한계라도 좋을 것이다) */
/* 레벨 1헤더는, 66x5. 레벨 2헤더는 65 x5, 레벨 3헤더는 65x5 */
...
/*
	64개라고 하는 것은 좋은 것인지?
	tek1h로 압축해서는 안된 것인지.
	tek1h는 길이 압축은 없고, 타입이나 베이스 주소는 s7로 저장. 다음에 계속 비트를 s7로(limit=0). 4 bit 단위로 lenbit 총수.
	256개 정도 있는 편이 좋을 것이다. 이것이라면 4KB블록으로, 레벨 1로 1MB, 레벨 2로 256MB가 된다.
	모드, 베이스 주소, 계속 비트는 s7, 최초의 사이즈와 차이 분 사이즈는 uc
*/
/* work는 lzcompress_tek1h가 3KB 사용한다 */
{
	int i, j, k, l, m, *s256;
	UCHAR *p, *q, *r, *s, *t, *u;
	tek1_s7ptr = outbuf;
...
//		tek1_puts7(srcsiz);
	...
	tek1_puts7s(i << 1 | 1); /* 상위 3bit는 버전(0은 tek1s만을 의미한다, 게다가 레벨 0사이즈 정보 없음) */
	/* 하위 1bit에 대해：0(슈퍼 short) */
	/* 001:normal long,011:멀티 모드 long, 그 외는 리저브 */
//	tek1_puts7(3); /* 3:tek1, 2:무압축 */
//	tek1_puts7(1); /* 터미네이터 */
	p = tek1_s7ptr;
	r = q = &p[322 * 1024]; /* 레벨 0 출력용 */
	...
	for (j = 0; j < i; j++, s += bsiz) {
		...
		if (&src[srcsiz] >= &s[bsiz]) {
			for (k = 0; k < j; k++, t += bsiz) {
				...
				/* 일치한 블록 발견 */
				s256[j] = s256[k]; /* k번째의 블록 내용을 그대로 사용해라 */
				goto skip1;
	skip0:
				;
			}
...
skip3:
	...
	if (i > 1) {
		tek1_s7ptr = p;
		tek1_puts7s((m = s - t) - 2); /* 레벨 1 zone 사이즈 */
		p = tek1_s7ptr;
		for (l = 0; l < m; l++)
			p[l] = t[l];
		p += m;
	...
	if (i == 1 && (flags & 1) ! = 0)
		...
		/* ver에 따라서는 이하 처리를 한다 */
	//	tek1_s7ptr = p;
	//	tek1_puts7((m = q - r) - 2); /* 레벨 0 zone 사이즈 */
	//	p = tek1_s7ptr;
	}
	m = q - r;
	for (l = 0; l < m; l++)
...
/* bit3-7:max(4, 8, 12, 16, 20, 24), bit0-2:mode(0, 1, 7) */
{
	...
	if (m < 7) {
		...
		if (i > 0 && ucstr->lentbl[i - 1] >= max)
			max += 4; /* max가 2번 이상 출현하는 것을 피하기 위해서 */
...
int calclen_uc0str(struct STR_UC *ucstr, int i, int n)
{
	...
	max = (max >> 3) & 0x1f;
	if (m == 7)
		l = (3 + 2 + 2) + ucstr->maxlen * 2; /* 부호 bit 출력 회수를 미리 더해 놓는다(maxlen + 1) 개 */
	if (m == 1)
		l = (2 + 1 + 1) + ucstr->maxlen; /* m == 0 때에 비해 1 긴 것을 더해 놓는다 */
...
/* 최후만 작은 것에는 잘 대응하지 못하고, m=7로 해 버린다 */
{
	UCHAR m, max;
	int d;
	max = m = calclen_ucstr_sub(ucstr, i);
...
fin:
	if (m < 7 && n > 0 && ucstr->maxlen > 0)
		putbitbuf(bitbuf, 1, 1); /* 최종 파라미터 보정치 */
	return;
}
...
void optsub_fullopt1(int i, int rest, struct STR_STATISTICS *stat0, int total0)
{
	...
	for (j--; j >= 0; j--) {
		if (i > 1 && optsub_ucstr->lentbl[i - 2] > j)
			continue; /* 시간이 걸리므로 하지 않는다 */
		base1 = base0 + (1 << j);
		tmp = 0;
		for (stat = stat0; stat->code < base1; stat++)
			tmp += stat->count;
		total1 = total0 + (i + j) * tmp;
		optsub_ucstr->maxlen = i - 1; /* 여기서 일단 중지해 본다 */
...
int optsub_evaluate1(int rest, struct STR_STATISTICS *st, int base0, int *pj, int k0, int mode)
/* 상당한 평가 함수 */
/* 제로 없음 모드로, 현재비 0행이 아닌 진행(mode=0) */
/* 혹은 제로 있어 모드(mode=-1) */
{
	int base1, k, eval = 0, tmp, j = 0;
	do {
...
/* count로 소트도 된다 */
{
	...
	stat[256]. count &= 0;
	for (i = k; i < 257; i++)
		stat[i]. count &= 0; /* 존재 판정에 사용하므로 제로 클리어 */
//	stat[k]. count = 0; /* 터미네이터 */
	/* 현명하지 않은 소트 */
	/* 우선 non zero만으로 소트 하고 있으므로 조금은 빠르다 */
	for (i = 0; i < k; i++) {
		max = stat[i]. count;
		max_j = i;
		for (j = i + 1; j < k; j++) {
...
//	stat[k]. count = 0; /* 터미네이터 */
	...
	for (i = 0; i < k; i++) {
		...
		stat[max_j]. code = stat[i]. code;
		stat[i]. code = j;
		trt[j] = i;	/* 외부 코드를 내부 코드로 변환하는 테이블 */
	}
	for (i = 0; i < 288; i++) {
		invtrt[i] = stat[i]. code; /* 내부 코드를 외부 코드로 변환하는 테이블 */
		stat[i]. code = i;
	}
	return k;
}
...
/* 매우 현명하지 않는 방법으로 출력시키자 우선 */
	return 1 + 4 + 4 + 8 + (tek1_intlog2p(max + 1) << 8); /* fix 이후 */
//	return 3 + 5 + 3 + 2 + 2 + 8 + (intlog2p(max + 1) << 8);
		/* 이용 레벨이 14까지 억제 당하면, 이것은 131바이트가 된다 */
		/* 이용 레벨이 6까지 억제 당하면, 이것은 99바이트가 된다 */
/*
255를 출력
...
//	return lzcompress_tek1s(256, table, 512, outtmp, sizeof work, work, 0x02 | intlog2m(max + 1) << 8, 12, 256);
	/* 이 때에는 bit수로 반환되고 실제 출력은 되지 않는다 */
	/* 이것으로 알 수 있듯이 부호류에서는 bytable이 최초로 출력된다.  */
}
...
void tek2_initucstr(UCHAR *table_lv, struct STR_UC *ucstr, int fix)
{
	...
	ucstr->maxlen = i - 1 - fix; /* 제어 비트(구len)의 최고 길이 */
...
//	for (i = 0; i < 256 && tek2_stat_calclen_ucstr_by[i]. count > 0; i++) {
	for (i = 0; tek2_stat_calclen_ucstr_by[i]. count > 0; i++) {
		j = tek2_table_tr0[i]; /* 내부 코드 i는 문자 j로 변환된다 */
		tmp[j] = i;
	}
	for (i = 0; i < 256; i++)
		trt[i] = tmp[i];
...
int output_lifetime(struct STR_BITBUF *btout, int lt0, int lt1, int len, int def0)
{
	if (lt1 < 0) {
		...
		if (lt0 >= len) {
			if (len < def0)
				lt0 = def0; /* 너무 작아 x1로 끌어올린다 */
			if (def0 < len && len <= def0 * 2)
				lt0 = def0 * 2; /* x2로 끝난다면 x2로 한다 */
		}
		if (lt0 == def0)
...
/* work는 3KB필요(int가 32bit) */
{
	...
	if (srcsiz >= 2) {
		...
		/* s256의 srcsiz-1개의 int에 대해서 encode한다 */
...
/* seek는 없다. 왜냐하면 일치했는지 어떠했는지로 동작을 바꿀 필요가 있기 때문에 */
/* 그렇지만 sbuf0를 NULL로 해도 괜찮다고 할까 */
...
void lzsub_stat1init(struct STR_LZSUB_STAT1 *work, UCHAR *src0, UCHAR *src1, int prm, int maxdis, int *table, int lv)
{
	...
	if (maxdis < srcsiz) {
		...
		maxdis = work->maxdis_r * LZSUB_STAT0_RESO; /* 끝수 보정 */
	...
	for (i = 0; i < work->btrees; i++) {
		...
		if (p == NULL)
			exit(1);
		add_free1(&work->btree[i], (struct STR_BT_NODE1 *) p); /* 텅 빈 것을 나타내기 위해 1개만 만들어 둔다 */
	}
	for (i = 0; i < 256; i++)
		work->l1table[i] = NULL; /* 한 글자 일치 고속 검색 테이블 */
	for (i = 0; i < 4; i++)
		work->repdis[i] |= -1;
...
/* sbuf0 == NULL라고 읽어 진행해 검색 버퍼를 갱신할 뿐으로 출력은 하지 않는다 */
{
...
printf("[%d %d] ", j, len);
		...
		if (len > src1 - src)
			len = src1 - src; /* 이 범위에서라면 dis를 개량할 수 있을 가능성은 있지만, 귀찮아서 생략(search0c를 개조하면 된다) */
		srchloglen = -1;
		if (len >= 2) {
...
/* sbuf에 슬라이드 사전의 기본 정보를 출력 */
/* lv는 0:1바이트만,1:16바이트,2:전부가 있다.  */
/* btree를 사용하지만, 버그 회피를 위해서 delete는 하지 않는다. delete의 버그가 회복되면, lzsub_stat1를 만들고 싶다.  */
/* work는 최대의 maxdis:2 MB에 대해서 과연 얼마나 필요한가?  */
{
...
/* 압축 고속화를 위한 Btree */
/* 노드 삭제에 즈음하여 pval를 수정하지 않아도 된다고 가정 */

void init_btree(struct STR_BTREE *btree)
{
	btree->level = 0; /* top은 세지 않는다 */
	btree->top = NULL;
	btree->free0 = NULL;
	btree->free1 = NULL;
	return;
...
skip1:
	...
	return;
//	return matchlen_bt(s, handle->pkey, s1); /* 최장 일치 길이(next가 최장 일치일지도 모르지만) */
}
...
/* node는 최소치가 바뀜, lv에 속하고 있음, 부모가 취해 줌 */
{
	struct STR_BT_NODE1 *node1 = node;
...
//if (debug==14274) printf("pass1:");
	...
	if (max < TEK1_BT_NODESIZ) {
		...
		node1->pkey[i] = s;
		if (i == 0 || i == max)
			node1->skiplen = matchlen_bt(node1->pkey[0], node1->pkey[max], s1); /* 부모가 있으면 부모의 skiplen를 이용하면 더 좋다(부모의 오른쪽 끝의 아이는 이용 불능인데 주의) */
		if (i > 0) {
//fputc(', ', stdout);
//if (debug > 14000) fputc('o', stdout);
		...
		while (lv >= 0) {
			...
			if (i > 0)
				break;
			node0->skiplen = matchlen_bt(node0->pkey[0], node0->pkey[node0->nodes - 1], s1); /* 부모 이용 가능 */
			lv--;
		}
//if (debug > 14000) fputc('o', stdout);
//fputc('e', stdout);
...
err:
		...
		node1->skiplen = matchlen_bt(node1->pkey[0], node1->pkey[TEK1_BT_NODESIZ / 2 - 1], s1); /* 친이용 가능 */
...
//if (debug==14274) printf("pass3:");
	if (max < TEK1_BT_NODESIZ) {
		...
		if (i == 0 || i == node0->nodes)
			node0->skiplen = matchlen_bt(node0->pkey[0], node0->pkey[node0->nodes], s1); /* 부모 이용 가능 */
		node1->skiplen = matchlen_bt(node1->pkey[0], node1->pkey[TEK1_BT_NODESIZ / 2 - 1], s1); /* 부모 이용 가능 */
		free1->skiplen = matchlen_bt(free1->pkey[0], free1->pkey[TEK1_BT_NODESIZ / 2 - 1], s1); /* 부모 이용 가능 */
		node0->nodes++;
		goto retry;
	}
	lv--;
	...
	while (lv >= 0) {
		...
		if (node00->nodes < TEK1_BT_NODESIZ) {
			...
			if (i == 0 || i == node00->nodes)
				node00->skiplen = matchlen_bt(node00->pkey[0], node00->pkey[node00->nodes], s1); /* 부모 이용 가능 */
			node0->skiplen = matchlen_bt(node0->pkey[0], node0->pkey[TEK1_BT_NODESIZ / 2 - 1], s1); /* 부모 이용 가능 */
			free0->skiplen = matchlen_bt(free0->pkey[0], free0->pkey[TEK1_BT_NODESIZ / 2 - 1], s1); /* 부모 이용 가능 */
			node00->nodes++;
			goto retry;
		}
		lv--;
	...
	for (;;) {
		...
		if (len >= len0) {
			...
			if (mode == 0) {
				...
				if (tl < len0) {
					/* lz == 1이다 */
					td++; /* 그 정도라면 바꾸지 않는 편이 나은 케이스가 많으니까 */
						/* 조건을 엄격하게 한다 */
				}
				if (td <= len && len <= len0 + 4 && dis < 64)
					break; /* 적어도 1바이트는 이득을 볼 것이다 */
...
/* opt:0 -- 압축율 우선 */
/* opt:1 -- 전개 속도 우선 */
/* 지금은 이제 opt == 0으로도 전개 속도와 압축율을 양립시키고 있다 */
/* 우선, work는(srcsiz + 257) * 2 * sizeof (int)로 좋은 것 같다.  */
{
	/* ds3:cp4 */
	static int table[11] = {
	...
	tek1_s7ptr = outbuf;
	p = src;
	if (*sbuf == 3)	/* 갑자기 fdis 변환 */
		sbuf += lzsub_decstat(sbuf, &dis, &len);
	i = lzsub_decstat(sbuf, &dis, &len);

	*tek1_s7ptr++ = 0x01; /* flen+스페셜 캐릭터 */
	while (i > 0) {
		...
		*tek1_s7ptr++ = kk << 4 | jj;
		if (jj == 0)
			tek1_puts7s(j); /* 패딩하고 싶을지도 모르기 때문에 s7s로 */
		if (kk == 0)
			tek1_puts7s(k); /* 패딩하고 싶을지도 모르기 때문에 s7s로 */
		do {
			*tek1_s7ptr++ = *p++;
		} while (--j);
		sbuf = t;
		...
		do {
			/* ds3:cp4 */
			jj = tek1_intlog2p(dis + 1); /* dis를 저장하는데 필요한 bit수 */
			for (kk = 0; jj - kk > 3; kk += 7); /* 하위 kkbit는 s7s에 맡긴다 */
			jj = len - 1;
			if (jj >= 16)
				jj = 0;
			*tek1_s7ptr++ = (kk == 0) | (dis >> kk) << 1 | jj << 4;
			if (kk > 0) {
				/* puts7s라면 상위의 0을 생략 할 수도 있는 것으로 자기 앞으로 */
				if (kk > 28)
...
/* opt:0 -- 압축율 우선 */
/* opt:1 -- 전개 속도 우선 */
/* 지금은 벌써 opt == 0으로도 전개 속도와 압축율을 양립시키고 있다 */
{
	/* ds3:cp4 */
	static int table[11] = {
	...
	if (hint == NULL)
		...
		if (loadfdis(srcsiz, fdstbl))
			fdstbl = NULL; /* 어떤 트러블이 있었는지, 혹은 결국 차이분 필요없음 */
	}
	...
	if (complev >= 5)
		i = 2;
	if (fdstbl)
		setsubbuf(srcsiz, src, fdstbl, subbuf); /* fdis는 최고로 65535 */
...
//for (sbuf = sbuf0; ;) { i = lzsub_decstat(sbuf, &dis, &len); printf("[%d %d] ", dis, len); sbuf += i; if (i == 0) break; }
	...
	cbylz[0] = cbylz[1] = 0;
//	hbylz[0] = hbylz[1] = 1; /* 최초의 1bit는 파기하지 않으면 안 되니까 */
	bylz[0] = bylz[1] = 0;
	fmode = (fdis ! = 0);
	...
	for (j = 0; j < i; j++, s += bsiz) {
		...
		if (&src[srcsiz] >= &s[bsiz]) {
			for (k = 0; k < j; k++, t += bsiz) {
				...
				/* 일치한 블록 발견 */
				s256[j] = s256[k]; /* k번째의 블록의 내용을 그대로 사용해라 */
				goto skip1;
	skip0:
				;
			}
...
skip3:
	...
	if (i > 1) {
		tek1_s7ptr = p;
		tek1_puts7s((m = s - t) - 2); /* 레벨1 zone 사이즈 */
		p = tek1_s7ptr;
		for (l = 0; l < m; l++)
			p[l] = t[l];
		p += m;
	...
	/* ver에 따라서는 이하를 한다 */
//	tek1_s7ptr = p;
//	tek1_puts7((m = q - r) - 2); /* 레벨0 zone 사이즈 */
//	p = tek1_s7ptr;
	m = q - r;
	for (l = 0; l < m; l++)
		p[l] = r[l];
...
근처를 건드리면 좋을 것이라고 생각된다 */
/* tek1_getlt0에도 IA-32에 의존한 기술이 있다 */

int tek1_intlog2p(int i)
/* 0~(i - 1)를 표현하는데 어떤 비트가 필요한가?  */
/* i = 1에 대해서 0을 반환한다 */
/* i = 0101에 대해서 3을 반환한다 */
/* i = 100에 대해서 2를 반환한다 */
{
...
/* alingn 위치까지 액세스가 거슬러 올라가는 일도 있을 수 있다 */
/* 또 최대로 종단+8바이트까지 액세스 할 수 있다 */
/* 모두 액세스 할 수 있는이라고 하는 것만으로, 데이터가 의미 있는 것일 필요는 없다 */
{
...
/* 1의 수를 센다 */
{
	static const UCHAR table[16] = {
		4, 0, 1, 0, 2, 0, 1, 0, 3, 0, 1, 0, 2, 0, 1, 0	/* 1로 정지한 경우 */
	};
	int i = 0, j;
	for (;;) {
		if (btbuf->rest < 32) {
...
lp:
	...
	if (c == 3) {
		m = 3; /* "000"이 2회 오면 l1c로 최대치 지정. 이것으로 무한대 대응.  */
		goto lp;
	}
	...
	for (;;) {
		...
		if (m == 3) {
			...
			if (n == 0 && s ! = 0)
				break; /* 0의 경우는 부의 부호가 나오는 것만으로 break; */
		}
		if (s == 0) {
			if (m == 0 && n > 0)
...
int tek1_lzrestore_tek1(int srcsiz, UCHAR *src, int outsiz, UCHAR *outbuf, int wrksiz, UCHAR *work, int flags)
/* 레벨 1까지 대응. 레벨 0의 경우 work는 사용하지 않는다 */
/* 레벨 1의 경우, work는 512*sizeof(int)만 사용한다 */
/* 이 루틴은 멀티 모드에도 대응하고 있다 */
/* srcsiz+8까지 액세스 하여 src보다 앞의 align까지는 액세스를 거슬러 올라가는 일이 있으므로 주의 */
/* 어느 쪽의 경우도 리드할 수 있는 것으로 그 값은 뭐든지 좋다 */
{
	...
	if (i & 1) {
		...
		i = 1;
		s7ptr = src;
		flags_tek2s |= 1; /* 쇼트 형식 인식 */
	}
	if (ver >= 4 * 256) {
err:
		return -1; /* 이해할 수 없는 버전 */
		/* 패딩을 넣어 두면 나중에 추가할 수 있다 */
		/* 그러나 ECC등이 들러붙으면 예외로 한다 */
		/* ECC등이 붙을지 어떨지는 ver의 bit1로 정해진다 */
	}
	ftyp[0] = 2; /* tek1s/tek2s(디폴트), 1:압축없음, 0:터미네이터 */
	/* s7s이 되었으므로 pad는 필요없다(마음대로 길게 할 수 있다) */
	if (ver & 1) {
...
fin:
	...
	if (i <= 256) {
		/* 레벨 1(32KB라면 이것이라도 8MB까지 OK) */
		j = tek1_getnum_s7s(&s7ptr) + 2;
		q = s7ptr;
		p = q + j;
		if (ver & 2)
			tek1_getnum_s7s(&p); /* 게다가 건너뛴다 */
		if (flags == 2)
...
void tek1_lzrestore_tek3h(int srcsiz, UCHAR *src, int i, int *s256, int ver, int *ftyp)
/* ver의 bit6를 1로 하면, 레벨2 이상의 헤더 해석에도 사용할 수 있다 */
{
	int j, k, l, m, n;
	UCHAR *s7ptr = src;
	if ((ver & 1) == 0) {
	...
	if (ver & 1) {
		...
		n = 0;
		do {
			k = tek1_getnum_s7s(&s7ptr); /* s7s로 하는 것은 의미 없고 길게 할 수가 있어 패드가 된다 */
			l = (k >> 2) & 0xff;
			m = (k & 0x03) | ((k >> 8) & ~0x03);
			if (l == 0)
				l = j;
	...
	s256[1] = tek1_getnum_s7s(&s7ptr); /* 여기에도 패드를 넣을 수 있다 */
	if (i > 1) {
		struct TEK1_STR_BTBUF btbuf;
...
/* ver의 bit6를 1로 하면, 레벨2 이상의 헤더 해석에도 사용할 수 있다 */
{
	...
	s256[1] = tek1_getnum_s7s(&s7ptr); /* 여기에도 패드가 넣을 수 있다 */
	if (i > 1) {
		n = 0;
		for (j = 1; j < i; j++) {
...
/* mode:	bit0:dec(0) /enc(1), bit1:auto(0) /force-full(1) */
{
	...
	static UCHAR header2[0x48] = {
		...
		"\x01\x01\x01\x01\x01\x01\x01\x01"
		"\x01\x01\x01\x01\x01\x01\x01\x01"
		"\x01\x01\x01\x01\x01\x01\x01\x01"	/* +0x2c : data begin */
		"\x6a\x20"
		"\x6a\xf1"
		"\x6a\x04"
		"\x89\xe3"
		"\x9a\x00\x00\x00\x00\xc7\x00"
		"\x83\xc4\x10"
		"\x01\x01\x01\x01\x01\x01"		/* +0x43 : entry */
	};
	static UCHAR header3[0x48] = {
		"\x2e\x8b\x62\x20"
		"\x6a\x21"
		"\xeb\x28"
		"GUIGUI00"
	...
	UCHAR flag = 0, ggflag = 0, ht = 0, tmp[56], *tp;
	int i = 0, j;
	unsigned int ti, tf;
	if (siz >= 0x20 && (p[16] == 0x00 || (siz >= 0x40 && p[0x3f] == 0x00))
		&& *(int *) &p[8] == ('G' | 'U' << 8 | 'I' << 16 | 'G' << 24)
		&& *(int *) &p[12] == ('U' | 'I' << 8 | '0' << 16 | '0' << 24))
		ggflag = 1;
	if (ggflag ! = 0 && siz >= 0x48) {
		if (p[0] == 0xf4 && (mode & 1) == 0 && 0x01 <= p[1] && p[1] <= 0x02) { /* f4복원 */
			if (p[0x2] ! = 0x80) {
				/* 0 x10-0 x2f 부분의 변환을 푼다 */
				for (j = 0; j < 6; j++)
					tmp[j] = p[j + 2];
				for (j = 6; j < 56; j++)
					tmp[j] = p[j + 10];
				for (j = 0; j < 56; j++)
					tmp[j] = tmp[j] << 1 | ((tmp[j] >> 7) ^ 0x01);
				tp = tmp;
				tf = tek1_getnum_s7s(&tp);
				ti = tek1_getnum_s7s(&tp) + 2;
				*(int *) &p[0x10] = ti << 12 | ti >> 20; /* stack+work */
				ti = tek1_getnum_s7s(&tp) + 4;
				*(int *) &p[0x20] = ti << 10 | ti >> 22; /* stack */
				ti = tek1_getnum_s7s(&tp) + 1;
				*(int *) &p[0x24] = ti <<  4 | ti >> 28; /* . data */
				ti = tek1_getnum_s7s(&tp) + 4;
				*(int *) &p[0x2c] = ti <<  4 | ti >> 28; /* img-location */
				if (tf & 1)
					*(int *) &p[0x14] = tek1_getnum_s7s(&tp);
				if (tf & 2) {
					ti = tek1_getnum_s7s(&tp) + 1;
					*(int *) &p[0x28] = ti <<  4 | ti >> 28; /* bss */
				}
			}
			if (p[1] == 0x01) {
				*(int *) &p[0x00] = 0x20628b2e;
				*(int *) &p[0x04] = 0x28ebff6a;
				*(int *) &p[0x30] = 0xf16a206a;
				*(int *) &p[0x34] = 0xe389046a;
				*(int *) &p[0x38] = 0x0000009a;
				*(int *) &p[0x3c] = 0x8300c700;
				p[0x40] = 0xc4;
				p[0x41] = 0x10;
			} else {
				*(int *) &p[0x00] = 0x20628b2e;
				*(int *) &p[0x04] = 0x28eb216a;
				*(int *) &p[0x30] = 0x046af16a;
				*(int *) &p[0x34] = 0x009ae389;
				*(int *) &p[0x38] = 0xc7000000;
				*(int *) &p[0x3c] = 0x0cc48300;
			}
		}
		flag = 1;
		ht = 0;
	...
	if (ggflag) {
		...
		if (mode == 1 && (p[20] & 2) == 0) {
			...
			p[20] &= 0x01;
			if (flag) {
				if (rjc(p + 0x20, p + i, 0x20, 0x20, i, 1)) /* encode */
					p[20] |= 0x06;
				if (ht == 1) {
					*(int *) &p[0x00] = 0x000001f4;
					*(int *) &p[0x04] = 0x00000000;
					*(int *) &p[0x30] = 0x00000000;
					*(int *) &p[0x34] = 0x00000000;
					*(int *) &p[0x38] = 0x00000000;
					*(int *) &p[0x3c] = 0x00000000;
					p[0x40] = 0x00;
					p[0x41] = 0x00;
				}
				if (ht == 2) {
					*(int *) &p[0x00] = 0x000002f4;
					*(int *) &p[0x04] = 0x00000000;
					*(int *) &p[0x30] = 0x00000000;
					*(int *) &p[0x34] = 0x00000000;
					*(int *) &p[0x38] = 0x00000000;
				...
				if (1 <= ht && ht <= 2) {
					...
					tek1_puts7s(tf);
					ti = *(int *) &p[0x10];
					tek1_puts7s((ti >> 12 | ti << 20) - 2); /* stack+work */
					ti = *(int *) &p[0x20];
					tek1_puts7s((ti >> 10 | ti << 22) - 4); /* stack */
					ti = *(int *) &p[0x24];
					tek1_puts7s((ti >>  4 | ti << 28) - 1); /* . data */
					ti = *(int *) &p[0x2c];
					tek1_puts7s((ti >>  4 | ti << 28) - 4); /* img-location */
					if (tf & 1)
						tek1_puts7s(*(int *) &p[0x14]);
					if (tf & 2) {
						ti = *(int *) &p[0x28];
						tek1_puts7s((ti >>  4 | ti << 28) - 1); /* . bss */
					}
					for (j = 0; j < 56; j++)
						tmp[j] = tmp[j] >> 1 | ((tmp[j] << 7) ^ 0x80);
					for (j = 0; j < 6; j++)
						p[j + 2] = tmp[j];
					for (j = 6; j < 54; j++)
						p[j + 10] = tmp[j];
				...
				if (rjc(p + 0x20, p + siz, 0x20, 0x20, siz, 1)) /* encode */
					p[20] |= 0x02;
			}
		}
	}
...
retry:
	...
	*q = '\0';
	...
	/* 선두 1바이트의 파기는, conv_tek5내에서 실시하므로 걱정할 필요없다 */
	free(q);
...
int tek_conv_tek5(int csiz, UCHAR *src, int osiz, UCHAR *tmp, UCHAR *dst, UCHAR *str_eprm)
{
	UCHAR *mclp0 = malloc(osiz * 8), *p, pmch, mcby, lastbyte, *mclp /* , *src1 = src + csiz */;
...
retry:
	...
	rc->bm[1]. lt = 0; /* 수명 없음 */
	tek_conv_tek5_setbm(&rc->bm[1], 0, 16); /* 불변형(항상1/2) */
	...
	if (testall[12] == 0) {
		...
		if (testall[12] == 1) {
			...
			eprm[EPRM_NST] = 2;
			testall[12] = 2;
			testall[13] = 3; /* 개조중의 파라미터 번호 */
		} else if (testall[12] == 2) {
			if (eprm[3] < 3)
				eprm[3]++;
			else {
				eprm[3] = testall[3];
			...
			if (i < 7) {
				...
				testall[12] = 4;
				testall[14] = 2;
				testall[13] = 14 + 4; /* z2a0b0c0d0 */
				for (i = 0; i <= 3; i++) {
					testall[16 + i] = eprm[i];
					eprm[i] = 0;
				}
			}
		} else if (testall[12] == 4) {
			testall[12] = 5;
			testall[13] = 14 + 3; /* z1a0b0c0 */
			eprm[EPRM_NST] = 1;
			for (i = 3; i <= 7; i++)
				eprm[i] = 0;
		} else if (testall[12] == 5) {
			testall[12] = 6;
			testall[13] = 14;
		//	eprm[EPRM_NST] = 1;
			for (i = 0; i <= 2; i++)
				eprm[i] = testall[16 + i]; /* 부활 */
		} else if (testall[12] == 6) {
			if (eprm[EPRM_NST])
				eprm[EPRM_NST]--;
			else {
				eprm[EPRM_NST] = testall[14];
				if (testall[14] <= 2) {
...
printf("d%de%df%dg%dh%d", eprm[3], eprm[4], eprm[5], eprm[6], eprm[7]);
				...
				} else if (testall[14] == 4) {
					eprm[EPRM_NST] = 2;
printf("eprm:z2a0b0c0d0e%df%dg%dh%d", eprm[4], eprm[5], eprm[6], eprm[7]);
					for (i = 0; i <= 3; i++)
						eprm[i] = 0;
				} else {
					eprm[EPRM_NST] = 1;
printf("eprm:z1a0b0c0");
					for (i = 0; i <= 3; i++)
						eprm[i] = 0;
				}
putchar('\n');
...
phaseloop:
	...
	/* 11-5그래서, 111111_00001_00000 */
	prb->repg3 = 0xffff; /* 자동 보정을 목표로 한다 */
	prb->repg4 = 0xffff;

	rc_init(rc, dst + (eprm[EPRM_NST] == 2) + 2); /* tek5이면 1바이트 margin를 연다 */
	rc->idnext = 0;
	rc->log1 = rc->log0;
	if (nst) {
		for (i = 2; i < 32; i++) {
		...
		if (eprm[EPRM_NST] == 1) {
			...
			rc->bm[5]. lt = 0; /* 수명 없음(repg3) --- 최초의 1bit를 아꼈다 */
			for (i = 0; i < MAX_PB2; i++) {
...
skip1:
	...
	dst[1] = pb * 45 + lp * 9 + lc;
	if (nst) {
		dst[1] = lp * 45 + pb * 9 + lc; /* lp가 좀처럼 1이상이 되지 않는 것을 이용 */
		if (eprm[EPRM_NST] == 2)
...
//printf("%d\n", i - r);
		if ((testall_z2_a > i - r) || (testall_z2_a == i - r && testall_z2_b < rc->range)) {
			...
			if (testall[13] < 14)
				testall[testall[13]] = eprm[testall[13]];
			else if (testall[13] == 14)
				testall[14] = eprm[EPRM_NST];
			else /* > 14 */
				testall[14] = testall[13] - 14; /* z2a0b0c0d0(4), z1a0b0c0(3) */
		}
		goto retry;
	}
...
/* 최초의 0x00이 있으므로 항상 +1한 바이트 수를 돌려준다 */
/* 미리 prob의 백업을 해 둘 것 */
/* total는 16-구total의 의미 */
{
	...
	if (total == 15) {
		...
		bytes = - bytes;
		*prange = ~bytes;
		return (bytes | 1) & 0x7ffffff1; /* 일단 그 나름대로 취급하기 쉬운 숫자로 보정 */
	}
#endif
	prbmax = -1 << (move + total);
	prbmin = ~prbmax;
	...
	prbmin &= total; /* 이것은 하위를 없애기 때문에 상위는 아무래도 좋다. ...당치도 않다, 상위도 중요 */
	prbmax &= total;
//	례：
//	move = 4, total=4 때
//	111111110001 0000
	...
	if (rc->phase == 0) {
		...
		if (bm->lt > 0) {
			...
			if (--bm->lt == 0) {
				/* 수명이 다했다 */
				if (bm->id ! = -1) {
					/* bm->logptr로부터 이것저것 산출해 rc->log0[]에 등록 */
					rc_substat(rc, bm);
				}
				bm->lt = bm->lt0;
				bm->id = rc->idnext++;
		...
		if (bm->lt > 0) {
			...
			if (--bm->lt == 0) {
				/* 수명이 다했다 */
				rc_encode1(rc, rc->fchglt, 1); /* 수명 변경 없음 */
				bound = 0;
...
int tek_lzrestore_tek5(int srcsiz, UCHAR *src, int outsiz, UCHAR *outbuf, UCHAR *mclp)
{
	...
	if (src == NULL)
		return 1;
	wrksiz = (0xc00 + (0x300 << (lc + lp))) * sizeof (UINT32); /* 최저 15KB, lc+lp=3이라면 36KB */
	work = malloc(wrksiz);
	if (work == NULL)
		return -1;
	...
	do {
		...
		err:
				...
				if ((bm->s = tek_rdget1(rd, &rd->fchgprm[(*prob >> 16) * 2 + bm->s], 1, 0)) == 0) {
					...
					if (bm->t + bm->m > 15)
						fprintf(stderr, "Warning: illeagal probe-model parameters.  (%d: %d-%d) \n", (*prob >> 16), bm->t, bm->m);
				}
				bm->lt = bm->lt0;
			}
			if (p < bm->prb0) {
...
int tek_decmain5(int *work, UCHAR *src, int osiz, UCHAR *q, int lc, int pb, int lp, int flags, UCHAR *mclp)
{
	...
	*q++ = tek_rdget1(rd, prb->lit, 8, 1) & 0xff;
	...
	do {
		s_pos = pos & m_pos;
		if (tek_rdget1(rd, &prb->pb[s_pos]. st[s]. mch, 1, 0) ^ stk) { /* 비lz */
			i = (q[-1] >> lcr | (pos & m_lp) << lc) << 8;
			s = state_table[s];
			if (pmch == 0)
				*q = tek_rdget1(rd, &prb->lit[i], 8, 1) & 0xff;
			else {
				j = 1; /* lit1는 최초부터 2를 줄이고 있다 */
				k = 8;
				pmch = q[rep[0]];
				do {
