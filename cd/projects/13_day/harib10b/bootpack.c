...
void HariMain(void)
{
	...
	sheet_setbuf(sht_win, buf_win, 160, 52, -1); /* 투명색없음 */
	...
	for (;;) {
		...
		if (fifo8_status(&keyfifo) + fifo8_status(&mousefifo) + fifo8_status(&timerfifo) == 0) {
			...
			if (fifo8_status(&keyfifo) != 0) {
				...
				i = fifo8_get(&timerfifo); /* 타임 아웃 한 것은 어떤 것일까?  */
				io_sti();
				if (i == 10) {
					...
					/* 0이나 1 */
					if (i != 0) {
						timer_init(timer3, &timerfifo, 0); /* 다음은 0을 */
						boxfill8(buf_back, binfo->scrnx, COL8_FFFFFF, 8, 96, 15, 111);
					} else {
						timer_init(timer3, &timerfifo, 1); /* 다음은 1을 */
